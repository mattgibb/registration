  ...
  def register_volumes(dataset, output_dir, image="")
    ...
    command = %{
      ...
      for image in #{image_list}
        do echo #{File.join PBS_DIR, 'register_volumes'} #{dataset} #{output_dir} --slice $image #{block_dir_flag} --stopAfterRigid | qsub -V -l walltime=0:010:00 -l select=1:mpiprocs=8 -N $image
      done}
  ...
  def build_colour_volume(dataset, output_dir)
    ...
    command = lambda do |flag|
      "echo #{File.join PBS_DIR, 'build_colour_volume'} #{dataset} #{output_dir} \
       ...
       | qsub -V -l walltime=0:15 -l select=1:mpiprocs=8 -N #{flag}vol"
    end
